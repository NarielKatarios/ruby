# [29.01.18 12:41]
# Друзья, мы с вами прошли основные конструкции в Ruby, также попробовали все это на практике отработать.
# Пришло время сделать небольшой проект самостоятельно. И таким проектом будет игра "Блек Джек" с текстовым интерфейсом.
# Понимаю, что заданим может быть сложным, поэтому цели завершить его полностью не ставится
# (хотя, будет отлично, если вы это сможете сделать).
# Важнее - правильно продумать классы и попытаться реализовать хотя бы часть механики игры (правила ниже),
# особенно ее логику.
#
...
# Начните с продумывания объектной модели - какие классы будут нужны, как они будут связаны, за что будут отвечать.
...
# Пользователю выдаются случайные 2 карты, которые он видит (карты указываем условными обозначениями,
...
#     Также пользователь видит сумму своих очков. Сумма считается по номианлу карт: от 2 до 10 - по номиналу,
...
# Используется, если сумма очков ниже 18. После этого ход переходит игроку.
# Может быть добавлена только одна карта.
#         Игроки вскрывают карты либо по достижению у них по 3 карты,
